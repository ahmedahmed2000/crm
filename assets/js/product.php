...
<script type="text/javascript">
    ...
    let forItemGroup = forID + '_group_contain'
    let forItemDesc = forID + '_desc_contain'
    var warehouse_id;
    var client_id;
    var supplier_id;
    ...
    $(document).ready(function () {
        ...
        $("#" + forItemCode).keyup(delay(function (request) {
            ...
            if (warehouse_id == undefined || warehouse_id == 0 || warehouse_id == '') {
                ...
                $.ajax({
                    url: '<?= base_url('admin/common/suggestionsName') ?>',
                    dataType: "json",
                    data: {
                        item_code: $("#" + forItemCode).val(),
                        item_group: $("#" + forItemGroup).val(),
                        item_desc: $("#" + forItemDesc).val(),
                        for: forItemCode,
                        warehouse_id: warehouse_id,
                        supplier_id: supplier_id,
                        client_id: client_id
        ...
        $("#" + forItemGroup).keyup(delay(function (request) {
            if (get(intStore)) {
                remove(intStore);
            }
            if (warehouse_id == undefined || warehouse_id == 0 || warehouse_id == '') {
        ...
        $("#" + forItemDesc).keyup(delay(function (request) {
            if (get(intStore)) {
                remove(intStore);
            }
            if (warehouse_id == undefined || warehouse_id == 0 || warehouse_id == '') {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        // $.each(data, function (i, obj) {
                        //     var item_id = obj.saved_items_id;
    ...
    function validate_qty(item_id) {
        if (pStore[item_id].total_qty) {
            if (qty_alert == 'Yes' && pStore[item_id].qty > pStore[item_id].total_qty) {
                //alert('<?//= lang('exceed_stock') ?>//' + ' ' + pStore[item_id].total_qty);
                // show message into child id in parent id in header
                $('#' + productTable).find('.item').each(function () {
                    if ($(this).attr('data-key') === item_id) {
                        $(this).addClass('danger');
    ...
    function init_items_sortable(preview_table) {
        var _items_sortable = $("body").find('.selectedItems tbody');
    ...
    function reorder_items() {
        var rows = $('.table.invoice-selected-items-table tbody tr.item,.table.table-main-estimate-edit tbody tr.item');
        var i = 1;
        $.each(rows, function () {
    ...
    function add_item_to_table(data, itemid, merge_invoice) {
        ...
        get_taxes_dropdown_template(tax_name, data.taxname).done(function (tax_dropdown) {
            ...
            table_row += '</td>';
            table_row += '<td class="bold item_name"><textarea style="width: fit-content;" name="items[' + item_key +
                '][item_name]" class="form-control">' + data.item_name + '</textarea></td>';
            table_row += '<td><textarea  style="width: fit-content;" name="items[' + item_key +
